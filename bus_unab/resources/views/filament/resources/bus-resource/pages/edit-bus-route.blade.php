@php
    ...
    $existingWaypoints = $record->routeWaypoints
        ->map(
            fn($w) => [
...
<x-filament-panels::page>
    ...
    <div wire:ignore x-data="googleBusRouteEditor(@js($allStops), @js($assignedStops), @js($existingWaypoints))" class="space-y-3">

        {{-- ══ TOOLBAR ══════════════════════════════════════════════════════════ --}}
        <div
            class="flex flex-wrap items-center justify-between gap-2 rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 px-4 py-2.5 shadow-sm">
            ...
            <div class="flex items-center gap-2">
                ...
                <button @click="mode = 'stop'"
                    :class="mode === 'stop' ? 'bg-indigo-600 text-white ring-indigo-600' :
                        'bg-gray-50 dark:bg-gray-700 text-gray-600 dark:text-gray-300 ring-gray-200 dark:ring-gray-600'"
                ...
                <button @click="mode = 'waypoint'"
                    :class="mode === 'waypoint' ? 'bg-orange-500 text-white ring-orange-500' :
                        'bg-gray-50 dark:bg-gray-700 text-gray-600 dark:text-gray-300 ring-gray-200 dark:ring-gray-600'"
                    class="inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-medium ring-1 transition-all">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                ...
                <button @click="mode = 'delete'"
                    :class="mode === 'delete' ? 'bg-red-600 text-white ring-red-600' :
                        'bg-gray-50 dark:bg-gray-700 text-gray-600 dark:text-gray-300 ring-gray-200 dark:ring-gray-600'"
                    class="inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-medium ring-1 transition-all">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
            ...
            <div class="hidden sm:flex items-center text-xs text-gray-400 dark:text-gray-500 italic">
                <span x-show="mode === 'stop'" style="display:none">Clic en parada gris → agregar &nbsp;·&nbsp; Clic en
                    número → quitar</span>
                <span x-show="mode === 'waypoint'" style="display:none">Clic en el mapa → añadir punto intermedio entre
                    paradas</span>
            ...
            <div class="flex items-center gap-2">
                <button @click="loadGoogleDirections()" :disabled="previewLoading || route.length < 2"
                    ...
                    <svg x-show="!previewLoading" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round"
                ...
                <button @click="saveAll()" :disabled="saving"
                    ...
                    <svg x-show="!saving" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2.5"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round"
        ...
        <div class="flex flex-col lg:flex-row gap-3" style="height: 650px;">
            ...
            <div
                class="w-full lg:w-72 flex-shrink-0 flex flex-col rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
                <div
                    class="px-3 py-2.5 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
                ...
                <div class="flex-1 overflow-y-auto divide-y divide-gray-100 dark:divide-gray-700/50" id="stop-list">
                    <template x-if="route.length === 0">
                        <div
                            class="flex flex-col items-center justify-center h-full text-center px-4 py-8 text-gray-400">
                            <svg class="w-10 h-10 mb-2 opacity-40" fill="none" viewBox="0 0 24 24"
                    ...
                    <template x-for="(stop, idx) in route" :key="stop.stop_id">
                        <div class="stop-list-item flex items-center gap-2 px-3 py-2">
                            {{-- Número --}}
                            <span
                                class="flex-shrink-0 w-6 h-6 rounded-full bg-indigo-600 text-white text-xs font-bold flex items-center justify-center"
                                x-text="idx + 1"></span>

                            {{-- Nombre --}}
                            <span class="flex-1 text-sm text-gray-700 dark:text-gray-200 truncate"
                                x-text="stop.name"></span>
                ...
                <div class="px-3 py-2 border-t border-gray-100 dark:border-gray-700 space-y-1 text-xs text-gray-400">
                    ...
                    <div class="flex items-center gap-1.5">
                        <span class="w-3 h-3 rounded-full bg-orange-500 flex-shrink-0"></span> Waypoint (ajuste de
                        ruta)
                    </div>
                </div>
            </div>
            ...
            {{-- ── Mapa ── --}}
            <div
                class="flex-1 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden relative">
                <div id="route-map-google" style="width:100%;height:100%;"></div>

                {{-- Badge de carga --}}
                <div x-show="previewLoading"
                    class="absolute top-4 left-1/2 -translate-x-1/2 bg-white dark:bg-gray-800 px-4 py-2 rounded-full shadow-lg border border-primary-500 flex items-center gap-2 z-10">
                    ...
                    <span class="text-xs font-medium text-gray-700 dark:text-gray-200">Calculando ruta con
                        Google...</span>
                </div>
            </div>
        </div>
        ...
        function googleBusRouteEditor(allStops, assignedStops, existingWaypoints) {
            return {
                ...
                setupMap() {
                    ...
                    allStops.forEach(s => bounds.extend({
                        lat: s.lat,
                        lng: s.lng
                    }));
                ...
                addWaypointAt(lat, lng, label) {
                    ...
                    this.waypoints.splice(insertIdx, 0, {
                        lat,
                        lng,
                        label,
                        _seg: seg,
                    ...
                    marker.addListener('dragend', () => {
                        ...
                        const {
                            seg: newSeg,
                            t: newT
                        } = this.findBestSegment(pos.lat, pos.lng);
                        ...
                        this.waypoints.splice(newIdx, 0, {
                            lat: pos.lat,
                            lng: pos.lng,
                            label: savedLabel,
                            _seg: newSeg,
                ...
                refreshWaypointSegments() {
                    ...
                    this.waypoints.forEach(wp => {
                        const {
                            seg,
                            t
                        } = this.findBestSegment(wp.lat, wp.lng);
                        wp._seg = seg;
                        wp._t = t;
                    });
                ...
                buildOrderedPath() {
                    ...
                    if (N === 0) return this.waypoints.map(w => ({
                        lat: w.lat,
                        lng: w.lng
                    }));
                    const path = [];
                ...
                findBestSegment(lat, lng) {
                    ...
                    for (let i = 0; i < N - 1; i++) {
                        const {
                            dist,
                            t
                        } = this.ptSegDist(lat, lng, this.route[i].lat, this.route[i].lng, this.route[i + 1].lat, this
                            .route[i + 1].lng);
